...
def load_volume_dataset():
    ...
    for i in range(1, 26):  # Assuming 25 images for biryani
        ...
        image=cv2.imread(image_path)# Load image using appropriate method (e.g., cv2.imread)
        biryani_images.append(image)
    
    # Load aloo paratha images
    for i in range(1, 26):  # Assuming 25 images for aloo paratha
        image_path = f'C:\\Users\\rahul\\Desktop\\TDL_Project\\Dataset\\Aloo_Paratha\\aloop_{i}.jpg'
        image=cv2.imread(image_path) # Load image using appropriate method (e.g., cv2.imread)
...
volume_model = Sequential([
    Conv3D(32, kernel_size=(3, 3, 3), activation='relu', input_shape=(64, 64, 64, 3)),
...
volume_model.save_weights("volume_estimation_model_weights.h5")
